...
const auctionConfigs: Array<{
    ...
    },
    {
        ...
        volume: 95,
        bodyType: "Рефрижератор",
        cargoName: "Замороженная продукция",
    },
    {
        type: "Request",
        status: "Active",
    ...
    },
    {
        ...
        volume: 55,
        bodyType: "Тент",
        cargoName: "Промышленное оборудование",
    },
    {
        type: "Down",
        status: "Active",
...
export function generateAuctions(): void {
    ...
    auctionConfigs.forEach((cfg, idx) => {
        ...
        const pricePerKm =
            cfg.price > 0 && distance > 0
                ? Math.round(cfg.price / distance)
                : null;
        ...
        const detail: AuctionDetail = {
            ...
            description:
                cfg.status === "Active"
                    ? `Перевозка груза: ${cfg.cargoName}. Требуется транспорт ${cfg.bodyType}.`
                    : null,
            organizer_name: `ООО "Грузоперевозки ${idx + 1}"`,
            organizer_rating: Math.round(Math.random() * 50) / 10 + 4,
            contact_person: cfg.hideContacts
                ? undefined
                : `Иван Иванович ${idx + 1}`,
            contact_phone: cfg.hideContacts
                ? undefined
                : `+7 (999) ${String(100 + idx).slice(1)}-${String(10 + idx).padStart(2, "0")}-${String(idx).padStart(2, "0")}`,
            ...
            max_price:
                cfg.type === "Up"
                    ? Math.round(cfg.price * 1.5)
                    : cfg.type === "Request"
                      ? Math.round(cfg.price * 1.2)
                      : null,
            start_price:
                cfg.type === "Down" || cfg.type === "Up" ? cfg.price : null,
            currency: "RUB",
    ...
    uuids.forEach((auctionUuid) => {
        ...
        for (let i = 0; i < betCount; i++) {
            ...
            const priceAdjustment =
                store.detail.auc_type === "Down" ? -(i * 5000) : i * 5000;
            ...
            const bet: AuctionBet = {
                ...
                comment:
                    i === 0 && isMine
                        ? "Готовы рассмотреть гибкие условия"
                        : null,
                status:
                    i === 0 && store.item.status === "Finished"
                        ? isMine
                            ? "Won"
                            : "Lost"
                        : "Active",
                cancel_reason: null,
                is_winner:
                    i === 0 && store.item.status === "Finished" && isMine,
                is_mine: isMine,
                rank,
...
export function addBet(
    ...
    store.detail.user_trading_status =
        store.detail.auc_type === "Down" ? "Losing" : "Leading";
    store.item.current_price = price;
    store.item.my_bet_exists = true;
    store.item.can_set_bet = false;
    store.item.user_trading_status =
        store.detail.auc_type === "Down" ? "Losing" : "Leading";
...
export function filterAuctions(params: {
    ...
    if (params.cargo_num) {
        items = items.filter((a) =>
            a.cargo_num.toLowerCase().includes(params.cargo_num!.toLowerCase()),
        );
    }
    if (params.status) {
