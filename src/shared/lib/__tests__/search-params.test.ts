...
function parseSearchParams(params: Record<string, unknown>) {
  return {
    ...
    perPage: params.per_page ? Number(params.per_page) : 20,
    ...
    isAvailable: params.is_available === true || params.is_available === 'true',
...
describe('search-params', () => {
  ...
  it('parses string filters', () => {
    const result = parseSearchParams({ cargo_num: 'CARGO-001', status: 'Active' });
    expect(result.cargoNum).toBe('CARGO-001');
    expect(result.status).toBe('Active');
  });

  it('handles price ranges', () => {
    const withPrices = parseSearchParams({ price_from: '1000', price_to: '5000' });
